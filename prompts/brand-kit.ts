...
- brandingMd: A complete brand guide in markdown covering brand name, tagline, domain, colors (with hex codes), fonts, tone of voice, brand personality, values, target audience, and logo prompt.
...
IMPORTANT: Return ONLY the JSON object, no markdown code fences, no extra text.`,
  ...
  user: (idea: string, industry: string, vibe: string, chosenName: string, chosenDomain: string) =>
    `Business idea: ${idea}
Industry: ${industry}
Brand vibe: ${vibe}
...
Generate the complete brand kit. Return ONLY valid JSON.`,
};
